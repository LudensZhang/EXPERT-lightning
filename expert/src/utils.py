...
def transfer_weights(base_model, init_model, reuse_levels, regresssion=False):
    init_model.statistics = base_model.statistics
    init_model.base = base_model.base
    init_model.base.requires_grad_(False)
    if regresssion:
        return init_model
    ...
    for layer, reuse_level in enumerate(reuse_levels):
        ...
        if use_level == 0:
            ...
            init_model.spec_inters[layer].requires_grad_(False)
        elif use_level == 2:
            init_model.spec_inters[layer] = init_model.spec_inters[layer]
            init_model.spec_integs[layer] = init_model.spec_integs[layer]
            init_model.spec_inters[layer].requires_grad_(False)
            init_model.spec_integs[layer].requires_grad_(False)
        elif use_level == 3:
            init_model.spec_inters[layer] = init_model.spec_inters[layer]
            init_model.spec_integs[layer] = init_model.spec_integs[layer]
            ...
            init_model.spec_inters[layer].requires_grad_(False)
            init_model.spec_integs[layer].requires_grad_(False)
            init_model.spec_outputs[layer].requires_grad_(False)
        else:
...
def read_labels(path, shuffle_idx, dmax=None, regression=False):
    # unk should be generated in map op, not here remember to fix
    if regression:
        labels = pd.read_hdf(path, key='rg').iloc[shuffle_idx, :]
        return labels 
