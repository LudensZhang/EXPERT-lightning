from expert.src.utils import load_otlg, meta_from_dir, map_to_ontology, meta_from_rg
import pandas as pd
...
def map(cfg, args):
    if args.from_dir:
        ...
        print('Generating table for metadata..')
        meta = meta_from_rg(args.input)
        print('Done')
        meta.to_hdf(args.output, key='rg', mode='a')
        
    else:
        raise ValueError('Please given one of the [from_dir, to_otlg, rg]')
